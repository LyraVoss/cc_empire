...
func (b *BrainEngine) GenerateResponse(ctx context.Context, userInput string, userContext map[string]interface{}) (string, error) {
	...
	profile, err := b.Fans.FetchProfile(ctx, userID)
	if err != nil {
		log.Printf("⚠️  BrainEngine: Profile Error for %s: %v", userID, err)
	}

	// Integrate profile data into context for NervousSystem
	userContext["nsfw_warnings"] = profile.NSFWWarnings
	userContext["is_blocked"] = profile.IsBlocked
	userContext["relationship_depth"] = profile.RelationshipDepth
	userContext["tier"] = profile.Tier
